...
# Prints a usable sysfs node for controlling Acer battery calibration mode.
...
shopt -s nullglob
candidates+=(/sys/bus/wmi/drivers/*/calibration_mode)
candidates+=(/sys/bus/platform/drivers/*/calibration_mode)
candidates+=(/sys/devices/platform/*/calibration_mode)
shopt -u nullglob
...
# Fallback: brute-force scan under /sys for calibration_mode (can be slower)
...
while IFS= read -r p; do
  ...
  echo "$p"
  exit 0
done < <(find /sys -maxdepth 6 -type f -name calibration_mode 2>/dev/null)

if [[ "$mode" == "writable" ]]; then
  echo "Could not find a writable calibration_mode sysfs node." >&2
